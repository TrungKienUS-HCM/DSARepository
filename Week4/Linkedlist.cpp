...
{
    List* newLL = new List;
    if (newLL) newLL->p_head = newLL->p_tail = p_node;
    return newLL;
}
...
bool removeHead(List *&L)
{
    if (!L->p_head || !L->p_tail) return false;
    NODE* tmpDel = L->p_head;
    if (tmpDel->p_next) L->p_head = L->p_head->p_next;
    else L->p_head = L->p_tail = NULL;
    delete tmpDel;
    return true;
}
...
void removeTail(List *&L)
{
    ...
    NODE* currNode = L->p_head;
    if (currNode->p_next) 
    {
        while (currNode->p_next->p_next) currNode = currNode->p_next;
        L->p_tail = currNode;
...
{
    if (!L->p_head || !L->p_tail) return;
    NODE* findVal = L->p_head;
    NODE* preDel = findVal;
    while (findVal->p_next) 
    {
        if (findVal->p_next->key == val)
            break;
        preDel = findVal;
        findVal = findVal->p_next;
    }
    if (findVal->p_next) 
    {
        if (findVal == L->p_head)
...
void removeAfter(List *&L, int val)
{
    ...
    if (findVal) 
    {
        if (findVal->p_next) 
        {
            if (findVal->p_next == L->p_tail)
                removeTail(L);
            else 
            {
                NODE* tmpDel = findVal->p_next;
                findVal->p_next = findVal->p_next->p_next;
                delete tmpDel;
                tmpDel = NULL;
            }
        }
...
void removePos(List *&L, int data, int pos)
{
    ...
    if (currNode) 
    {
        if (currNode == L->p_head)
            removeHead(L);
        else if (currNode == L->p_tail)
            removeTail(L);
        else 
        {
            preCurr->p_next = currNode->p_next;
            delete currNode;
            currNode = NULL;
        }
    }
...
{
    if (!L->p_head || !L->p_tail) return false;
    NODE* pCheck = L->p_head;
    NODE* pPrev = pCheck;
    while (pCheck && pCheck->key != val)
    {
        pPrev = pCheck;
        pCheck = pCheck->p_next;
    }
    if (pCheck)
    {
        if (pCheck == L->p_head)
...
bool addAfter(List *&L, int data, int val)
{
    ...
    if (findVal)
    {
        ...
        else 
        {
            NODE* newNode = createNode(data);
            if (!newNode) return false;
            newNode->p_next = findVal->p_next;
            findVal->p_next = newNode;
        }
        return true;
    }
    return false;
...
{
    int cnt = 0;
    for (NODE* pRun = L->p_head; pRun; pRun = pRun->p_next)
        ++cnt;
    return cnt;
}
...
List* reverseList(List* L)
{
    if (!L || !L->p_head || !L->p_tail) return new List();
    List* ReversedList = new List();

    for (NODE* currNode = L->p_head; currNode; currNode = currNode->p_next)
    {
        addHead(ReversedList, currNode->key);
    }
...
void removeDuplicate(List *&L)
{
    if (!L->p_head || !L->p_tail || !L->p_head->p_next) return;
    for (NODE* currNode = L->p_head; currNode && currNode->p_next; currNode = currNode->p_next) 
    {
        NODE* pCheck = currNode->p_next;
        NODE* pPrev = currNode;
        while (pCheck)
        {
            NODE* pNext = pCheck->p_next;
...
bool removeElement(List *&L, int key)
{
    ...
    while (currNode) 
    {
        NODE* pCurrNext = currNode->p_next;
        if (currNode->key == key) 
        {
            if (currNode == L->p_head)
                removeHead(L);
