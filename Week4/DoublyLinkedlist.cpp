...
{
    d_List* DoubleList = new d_List();
    DoubleList->pHead = DoubleList->pTail = NULL;
    for (int i = 0; i < n; ++i)
        addTail(DoubleList, arr[i]);
    return DoubleList;
}
...
{
    d_NODE* pCheck1 = L1->pHead;
    d_NODE* pCheck2 = L2->pHead;

    while (pCheck1 && pCheck2) 
    {
        if (pCheck1->key != pCheck2->key)
        {
            return false;
        }
        pCheck1 = pCheck1->pNext;
        pCheck2 = pCheck2->pNext;
    }

    return (!pCheck1 && !pCheck2);
}

// function to free the memory of a d_List
void freeList(d_List* L)
{
    if (!L || !L->pHead || !L->pTail) return;
    while (L->pHead)
    {
        removeHead(L);
    }
...
bool addHead(d_List*& L, int data) 
{
    d_NODE* newHead = createNode(data);
    if (!newHead || !L) return false;
    if (!L->pHead || !L->pTail)
    {
...
bool addTail(d_List*& L, int data) {
    d_NODE* newTail = createNode(data);
    if (!newTail || !L) return false;
    if (!L->pHead || !L->pTail)
    {
        L->pHead = L->pTail = newTail;
    }
    else 
    {
        newTail->pPrev = L->pTail;
        L->pTail->pNext = newTail;
        L->pTail = newTail;
    }
    return true;
}
...
bool removeHead(d_List*& L) 
{
    if (!L || !L->pHead || !L->pTail) return false;
    d_NODE* tmpDel = L->pHead;
    L->pHead = L->pHead->pNext;
    if (!L->pHead) L->pTail = L->pHead;
...
{
    if (!L || !L->pHead || !L->pTail) return;
    d_NODE* tmpDel = L->pTail;
    L->pTail = L->pTail->pPrev;
    if (!L->pTail) L->pHead = L->pTail;
...
{
    if (!L || !L->pHead || !L->pTail) return;
    d_NODE* pFind = L->pHead;
    while (pFind && pFind->key != val)
    {
        pFind = pFind->pNext;
    }
    if (pFind && pFind->pPrev) 
    {
        if (pFind->pPrev == L->pHead)
            removeHead(L);
        else 
        {
            d_NODE* tmpDel = pFind->pPrev;
            pFind->pPrev = pFind->pPrev->pPrev;
            pFind->pPrev->pNext = pFind;
            delete tmpDel;
        }
    }
}
...
void removeAfter(d_List*& L, int val) 
{
    ...
    if (pFind && pFind->pNext) 
    {
        if (pFind->pNext == L->pTail)
            removeTail(L);
        else
        {
            d_NODE* tmpDel = pFind->pNext;
            pFind->pNext = pFind->pNext->pNext;
            pFind->pNext->pPrev = pFind;
            delete tmpDel;
        }
    }
}
...
bool addPos(d_List*& L, int data, int pos) 
{
    ...
    if (pFind) 
    {
        ...
        else
        {
            ...
            newNode->pNext = pFind;
            newNode->pPrev = pFind->pPrev;
            pFind->pPrev->pNext = newNode;
            pFind->pPrev = newNode;
        }
    }
    else
...
void removePos(d_List*& L, int data, int pos) 
{
    if (!L || !L->pHead || !L->pTail || pos < 0) return;
    d_NODE* pFind = L->pHead;
    while (pos && pFind)
    {
        pos--;
...
bool addAfter(d_List*& L, int data, int val) 
{
    ...
    if (pFind)
    {
        ...
        else
        {
            d_NODE* newNode = createNode(data);
            if (!newNode) return false;
            newNode->pNext = pFind->pNext;
            newNode->pPrev = pFind;
            pFind->pNext->pPrev = newNode;
            pFind->pNext = newNode;
        }
        return true;
    }
...
{
    if (!L || !L->pHead || !L->pTail) return;
    for (d_NODE* pRun = L->pHead; pRun; pRun = pRun->pNext)
        std::cout << pRun->key << ' ';
}

int countElements(d_List* L) 
{
    if (!L || !L->pHead || !L->pTail) return 0;
    int cnt = 0;
    for (d_NODE* pRun = L->pHead; pRun; pRun = pRun->pNext)
        ++cnt;
    return cnt;
...
d_List* reverseList(d_List* L) 
{
    if (!L || !L->pHead || !L->pTail) return new d_List();
    d_List* ReversedList = new d_List();
    for (d_NODE* pRun = L->pHead; pRun; pRun = pRun->pNext)
        addHead(ReversedList, pRun->key);
    return ReversedList;
}

void removeDuplicate(d_List*& L) 
{
    if (!L || !L->pHead || !L->pTail || !L->pHead->pNext) return;
    for (d_NODE* currNode = L->pHead; currNode; currNode = currNode->pNext)
    {
        d_NODE* pCheck = currNode->pNext;
        while (pCheck)
        {
            d_NODE* pNext = pCheck->pNext;
            if (pCheck->key == currNode->key)
            {
                if (pCheck == L->pTail)
                {
                    removeTail(L);
                ...
                {
                    pCheck->pPrev->pNext = pCheck->pNext;
                    pCheck->pNext->pPrev = pCheck->pPrev;
                    delete pCheck;
                    pCheck = NULL;
                }
            }
